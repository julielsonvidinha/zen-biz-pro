...
interface AP { id: string; description: string; amount: number; due_date: string; paid_date: string | null; status: string; payment_method: string | null; }
...
const ContasPagar = () => {
  ...
  const [form, setForm] = useState({ description: "", amount: 0, due_date: "", payment_method: "" });
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from("accounts_payable").insert({
      description: form.description, amount: form.amount, due_date: form.due_date,
      payment_method: form.payment_method || null, user_id: user!.id,
    });
  ...
  const markPaid = async (id: string) => {
    await supabase.from("accounts_payable").update({ status: "pago", paid_date: new Date().toISOString().split("T")[0] }).eq("id", id);
  ...
  return (
    <div className="animate-fade-in space-y-4">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center shrink-0"><CreditCard className="w-5 h-5 text-primary" /></div>
        <div className="flex-1"><h1 className="text-xl font-bold">Contas a Pagar</h1><p className="text-sm text-muted-foreground">Controle de pagamentos</p></div>
        {canWrite && <Button onClick={() => { setForm({ description: "", amount: 0, due_date: "", payment_method: "" }); setDialogOpen(true); }} className="gap-2"><Plus className="w-4 h-4" /> Nova Conta</Button>}
      </div>
              ...
              <tbody>
                {items.map(a => (
                  <tr key={a.id} className="border-b border-border/50 last:border-0 hover:bg-muted/30">
                    ...
                    <td className="p-3 text-center"><Badge variant={a.status === "pago" ? "default" : "destructive"} className="text-xs">{a.status}</Badge></td>
                    <td className="p-3">{a.status !== "pago" && canWrite && <Button variant="ghost" size="icon" onClick={() => markPaid(a.id)}><CheckCircle className="w-4 h-4 text-success" /></Button>}</td>
                  </tr>
                ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent><DialogHeader><DialogTitle>Nova Conta a Pagar</DialogTitle><DialogDescription>Preencha os dados</DialogDescription></DialogHeader>
